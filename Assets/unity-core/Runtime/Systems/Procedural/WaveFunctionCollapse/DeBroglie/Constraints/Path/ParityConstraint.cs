...
namespace DeBroglie.Constraints
{
    ...
    public class ParityConstraint : ITileConstraint
    {
        ...
        {
            var oddPathTiles = this.PathSpec.Exits.Where(x => x.Value.Count() % 2 == 1).Select(x=>x.Key).ToList();
            this.oddPathTilesSet = propagator.CreateTileSet(oddPathTiles);
            this.oddPathTracker = oddPathTiles.Count > 0 ? propagator.CreateSelectedTracker(this.oddPathTilesSet) : null;
            this.pathView = (EdgedPathView)this.PathSpec.MakeView(propagator);
        ...
        {
            var visited = new bool[this.topology.IndexCount];

            foreach(var i in this.topology.GetIndices())
            {
                this.VisitIndex(i, visited);
            }
        }
        ...
        public void VisitIndex(int index, bool[] visited)
        {
            ...
            while (stack.Count > 0)
            {
                ...
                var isOdd = this.oddPathTracker?.GetQuadstate(i) ?? Quadstate.No;
                ...
                for (var d = 0; d < this.topology.DirectionsCount; d++)
                {
                    var direction = (Direction)d;
                    var qs = this.pathView.TrackerByExit.TryGetValue(direction, out var tracker) ? tracker.GetQuadstate(i) : Quadstate.No;
                    if (qs.IsYes()) parityCount++;
                    if (qs.IsMaybe())
                    {
                        if (this.topology.TryMove(i, direction, out var i2))
                        {
            ...
            if (amigLocation != null)
            {
                ...
                this.topology.GetCoord(ambigIndex, out var x, out var y, out var z);
                if (ambigDirection == null)
                {
                    if (parityCount % 2 == 0)
                    {
                        this.propagator.Ban(x, y, z, this.oddPathTilesSet);
                    }
                    else
                    {
                        this.propagator.Select(x, y, z, this.oddPathTilesSet);
                    }
                }
                else
                {
                    if (parityCount % 2 == 0)
                    {
                        this.propagator.Ban(x, y, z, this.pathView.TileSetByExit[ambigDirection.Value]);
                    }
                    else
                    {
                        this.propagator.Select(x, y, z, this.pathView.TileSetByExit[ambigDirection.Value]);
                    }
                }
            }
            else
            {
                ...
                else
                {
                    ...
                    this.propagator.SetContradiction("Parity constraint detected unfixable region of odd parity", this);
                }
            }
        }
    }
