...
namespace DeBroglie.Models
{
    internal struct TileModelMapping
    {
        ...
        public void GetTileCoordToPatternCoord(int x, int y, int z, out int px, out int py, out int pz, out int offset)
        {
            ...
            var (point, _, o) = this.TileCoordToPatternCoordIndexAndOffset.Get(x, y, z);
            px = point.X;
            py = point.Y;
            pz = point.Z;
            offset = o;
        ...
        public TilePropagatorTileSet CreateTileSet(IEnumerable<Tile> tiles)
        {
            ...
            if (set.Tiles.Count == 1)
            {
                ...
                foreach (var o in this.TilesToPatternsByOffset.Keys)
                {
                    set.OffsetToPatterns[o] = this.TilesToPatternsByOffset[o].TryGetValue(tile, out var patterns) ? patterns : EmptyPatternSet;
                }
            }
            return set;
        }
        ...
        {
            return GetPatterns(this.TilesToPatternsByOffset[offset], tile);
        }
        ...
        public ISet<int> GetPatterns(TilePropagatorTileSet tileSet, int offset)
        {
            ...
            {
                var tilesToPatterns = this.TilesToPatternsByOffset[offset];
