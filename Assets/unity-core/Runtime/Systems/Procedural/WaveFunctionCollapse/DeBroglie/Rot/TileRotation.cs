...
namespace DeBroglie.Rot
{
    ...
    public class TileRotation
    {
        ...
        {
            if(this.rotationGroup != null && tile.Value is RotatedTile rt)
            {
                rotation = rt.Rotation * rotation;
                tile = rt.Tile;
            }

            if(this.rotations != null && this.rotations.TryGetValue(tile, out var d))
            {
            ...
            if (!this.treatments.TryGetValue(tile, out var treatment))
        ...
        public IEnumerable<Tile> Rotate(IEnumerable<Tile> tiles, Rotation rotation)
        {
            ...
            {
                if(this.Rotate(tile, rotation, out var tile2))
                {
                    yield return tile2;
                }
            }
        ...
        {
            foreach (var rotation in this.RotationGroup)
            {
                if (this.Rotate(tile, rotation, out var tile2))
                {
                    yield return tile2;
        ...
        {
            return tiles.SelectMany(tile => this.RotateAll(tile));
        }
        ...
        public Tile Canonicalize(Tile t)
        {
            ...
            {
                if (!this.Rotate(rt.Tile, rt.Rotation, out var result))
