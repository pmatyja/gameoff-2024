...
namespace DeBroglie.Rot
{
    ...
    {
        private Dictionary<Tile, SubGroup> tileToSubGroup = new();
        ...
        {
            this.rotationGroup.CheckContains(rotation);
            this.GetGroup(src, out var srcSg);
            this.GetGroup(dest, out var destSg);
            // Groups need merging
            if(srcSg != destSg)
            {
                ...
                {
                    this.Set(srcSg, kv.Key, kv.Value, $"record rotation from {src} to {dest} by {rotation}");
                    this.tileToSubGroup[kv.Value] = srcSg;
                }
            }
        ...
        public void AddSymmetry(Tile tile, TileSymmetry ts)
        {
            ...
            {
                case TileSymmetry.F:
                    this.GetGroup(tile, out var _);
                    break;
                case TileSymmetry.N:
                    this.Add(tile, new Rotation(2 * 90, false), tile);
                    break;

                case TileSymmetry.T:
                    this.Add(tile, new Rotation(0 * 90, true), tile);
                    break;
                case TileSymmetry.L:
                    this.Add(tile, new Rotation(1 * 90, true), tile);
                    break;
                case TileSymmetry.E:
                    this.Add(tile, new Rotation(2 * 90, true), tile);
                    break;
                case TileSymmetry.Q:
                    this.Add(tile, new Rotation(3 * 90, true), tile);
                    break;
                ...
                case TileSymmetry.Cyclic:
                    this.Add(tile, new Rotation(1 * 90, false), tile);
                    break;
        ...
        public TileRotation Build()
        {
            ...
            {
                var treatment = sg.Treatment ?? this.defaultTreatment;
                ...
                foreach(var r2 in this.rotationGroup)
                {
            ...
            return new TileRotation(this.tileToSubGroup.ToDictionary(kv => kv.Key, kv => GetDict(kv.Key, kv.Value)), this.tileToSubGroup.Where(kv=>kv.Value.Treatment.HasValue).ToDictionary(kv => kv.Key, kv => kv.Value.Treatment.Value), this.defaultTreatment, this.rotationGroup);
        }

        // Gets the rotation group containing Tile, creating it if it doesn't exist
        private void GetGroup(Tile tile, out SubGroup sg)
        {
            if(this.tileToSubGroup.TryGetValue(tile, out sg))
            {
                return;
            }
            ...
            do
            {
                ...
                foreach (var entry in sg.Entries)
                {
                    foreach (var kv in sg.Tiles.ToList())
                    {
                        ...
                        {
                            expanded = expanded || this.Set(sg, kv.Key * entry.Rotation, entry.Dest, "resolve conflicting rotations");
                        }
                        if (kv.Value == entry.Dest)
                        {
                            expanded = expanded || this.Set(sg, kv.Key * entry.Rotation.Inverse(), entry.Src, "resolve conflicting rotations");
                        }
                    }
                }
            } while (expanded);
        ...
        private void Generate(SubGroup sg)
        {
            ...
            for (var refl = 0; refl < (this.rotationGroup.ReflectionalSymmetry ? 2 : 1); refl++)
            {
                for (var rot = 0; rot < 360; rot += this.rotationGroup.SmallestAngle)
                {
                    ...
                    for (var refl2 = 0; refl2 < (this.rotationGroup.ReflectionalSymmetry ? 2 : 1); refl2++)
                    {
                        for (var rot2 = 0; rot2 < 360; rot2 += this.rotationGroup.SmallestAngle)
                        {
        ...
        private class SubGroup
        {
            ...
            {
                return this.Tiles.Where(kv => kv.Value == tile).Select(x => x.Key).ToList();
            }
        
            public void Permute(Func<Rotation, Rotation> f)
            {
                this.Tiles = this.Tiles.ToDictionary(kv => f(kv.Key), kv => kv.Value);
            }
        }
