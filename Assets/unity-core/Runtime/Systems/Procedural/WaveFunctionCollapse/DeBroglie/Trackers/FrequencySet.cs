    ...
    internal class FrequencySet
    {
        ...
        public FrequencySet(double[] weights, int[] priorities = null)
        {
            ...
            for (var i = 0; i < weights.Length; i++)
            {
                ...
                this.frequencies[i] = f;
                this.plogp[i] = this.ToPLogP(f);
                frequenciesByPriority[priority].Add(f);
                group.plogp.Add(this.ToPLogP(f));
            }
            ...
            this.groups = groupsByPriority.OrderByDescending(x => x.Key).Select(x => x.Value).ToArray();
            var priorityToPriorityIndex = this.groups.Select((g, i) => new { g, i }).ToDictionary(t => t.g.priority, t => t.i);
            this.priorityIndices = priorities.Select(p => priorityToPriorityIndex[p]).ToArray();
        }
